					...
					{#if searchEnabled}
						<div class="my-0.5 flex ml-2 mr-0.5 {$mobile ? 'h-11' : 'h-[1.6875rem]'} shrink-0 items-center gap-2">
							...
							<input
								...
								class="input-plain w-full bg-transparent text-[0.8125rem] font-normal outline-hidden placeholder:text-gray-400 dark:placeholder:text-gray-500"
									...
									{#if modelFilterItems.length > 0}
										<TagSelector
											...
											false)
												? 'text-gray-700 hover:bg-gray-200 hover:text-gray-900 dark:text-gray-300 dark:hover:bg-gray-800 dark:hover:text-gray-100'
												: 'text-gray-500 hover:bg-gray-50/40 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-800/40 dark:hover:text-gray-100'}"
											itemClass="flex {$mobile ? 'h-11' : 'h-[1.6875rem]'} w-full cursor-pointer items-center gap-2 rounded-xl bg-transparent px-2 text-[0.8125rem] capitalize {($settings?.highContrastMode ??
											false)
							...
							{#if target.download}
								...
								>
									<div
										...
										class="flex {$mobile ? 'h-11' : 'h-8'} w-full select-none items-center gap-2 rounded-xl px-2 text-left text-[0.8125rem] font-normal text-gray-700 outline-hidden transition-colors duration-75 dark:text-gray-100 {selectedModelIdx ===
							...
							{:else}
								<Tooltip content={target.actionLabel} placement="top-start">
									<button
										...
										aria-selected={selectedModelIdx === filteredItems.length + targetIndex}
										data-arrow-selected={selectedModelIdx === filteredItems.length + targetIndex}
										class="focus-ring flex {$mobile ? 'h-11' : 'h-8'} w-full cursor-pointer select-none items-center gap-2 rounded-xl px-2 text-left text-[0.8125rem] font-normal text-gray-700 outline-hidden transition-colors duration-75 dark:text-gray-100 {($settings?.highContrastMode ??
										false)
