...
{#if $showSidebar}
	...
	>
		...
		>
			...
			>
				<a
					class="flex items-center rounded-xl {mobile ? 'size-9' : 'size-8.5'} h-full justify-center hover:bg-gray-50 dark:hover:bg-gray-900 transition no-drag-region"
					href="/"
					draggable="false"
				...
				>
					<img
						...
						src="{WEBUI_BASE_URL}/static/favicon.png"
						class="sidebar-new-chat-icon {mobile ? 'size-5.5' : 'size-5'} rounded-full"
						alt=""
					/>
				</a>
				...
				>
					...
					>
						<div class=" self-center">
							<Sidebar className="{mobile ? 'size-4.5' : 'size-4'}" />
						</div>
					</button>
				</Tooltip>
			...
			>
				<div class="pb-1">
					<div class="px-1 flex justify-center text-gray-700 dark:text-gray-300">
						<a
							id="sidebar-new-chat-button"
							class="group grow flex items-center space-x-2 rounded-xl {mobile ? 'px-3 py-2.5' : 'px-2 py-1.5'} hover:bg-gray-50 dark:hover:bg-gray-900 transition outline-none"
							href="/"
							draggable="false"
							on:click={newChatHandler}
							aria-label={$i18n.t('New Chat')}
						>
							<div class="self-center flex {mobile ? 'size-5' : 'size-4'} shrink-0 items-center justify-center">
								<EditPencilIcon className="{mobile ? 'size-5' : 'size-4'}" strokeWidth="1.5" />
							</div>

							<div class="flex flex-1 self-center translate-y-[0.5px]">
								<div class=" self-center {mobile ? 'text-[14px] leading-5' : 'text-[13px] leading-5'}">{$i18n.t('New Chat')}</div>
							</div>
					...
					<div class="px-1 flex justify-center text-gray-700 dark:text-gray-300">
						...
						>
							...
							<div class="flex flex-1 self-center translate-y-[0.5px]">
								<div class=" self-center {mobile ? 'text-[14px] leading-5' : 'text-[13px] leading-5'}">{$i18n.t('Search')}</div>
							</div>
							...
							{#if meta && isMenuItemVisible(itemId)}
								...
								>
									<a
										...
										class="grow flex items-center space-x-2 rounded-xl {mobile ? 'px-3 py-2.5' : 'px-2 py-1.5'} transition {itemId ===
										activeMenuItemId
											? ($settings?.highContrastMode ?? false)
												? 'bg-black/[0.035] dark:bg-white/[0.06]'
												: 'bg-black/[0.035] dark:bg-white/[0.045]'
									...
									>
										...
										<div class="flex self-center translate-y-[0.5px]">
											<div class=" self-center {mobile ? 'text-[14px] leading-5' : 'text-[13px] leading-5'}">{$i18n.t(meta.label)}</div>
										</div>
									</a>
								</div>
							{/if}
				...
				<div class="flex flex-col">
					{#if $user !== undefined && $user !== null}
						...
						>
							<button
								type="button"
								class=" flex items-center rounded-xl {mobile ? 'py-2.5 px-2.5' : 'py-1.5 px-1.5'} w-full hover:bg-gray-50 dark:hover:bg-gray-900 transition"
								aria-label={$i18n.t('User menu')}
							>
								<div class=" self-center mr-3 relative flex-shrink-0">
									<img
										src={`${WEBUI_API_BASE_URL}/users/${$user?.id}/profile/image`}
										class="{mobile ? 'size-6.5' : 'size-5.5'} object-cover rounded-full"
										alt={$i18n.t('Open User Profile Menu')}
										aria-label={$i18n.t('Open User Profile Menu')}
									/>
								...
								<div class=" self-center font-normal truncate {mobile ? 'text-[14px]' : ''}">{$user?.name}</div>
							</button>
						</UserMenu>
					{/if}
				</div>
