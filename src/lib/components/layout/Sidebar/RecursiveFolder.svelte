	...
	>
		...
		<div class="w-full group">
			<div
				...
				class="relative w-full {$mobile ? 'py-2 px-2' : 'py-1 px-1.5'} rounded-xl flex items-center gap-1.5 hover:bg-gray-50/40 dark:hover:bg-gray-800/40 transition {$selectedFolder?.id ===
				folderId
			...
			>
				<button
					class="text-gray-600 dark:text-gray-400 transition-all {$mobile ? 'p-1.5' : 'p-1'} hover:bg-gray-50/40 dark:hover:bg-gray-800/40 rounded-lg"
					on:click={(e) => {
						e.stopPropagation();
				...
				>
					{#if folders[folderId]?.meta?.icon}
						<div class="flex group-hover:hidden transition-all">
							<Emoji className="{$mobile ? 'size-4' : 'size-3.5'}" shortCode={folders[folderId].meta.icon} />
						</div>
					...
					{:else}
						<div class="flex group-hover:hidden transition-all">
							<FolderIcon className="{$mobile ? 'size-4' : 'size-3.5'}" strokeWidth="1.5" />
						</div>
				...
				<div class="translate-y-[0.5px] flex min-w-0 flex-1 items-center gap-1.5 pr-6 text-start">
					{#if edit}
						...
						/>
					{:else}
						<div class="min-w-0 truncate {$mobile ? 'text-[14px]' : ''}">
							{folders[folderId].name}
						</div>
				...
				{#if !folders[folderId]?.shared || folders[folderId]?.permission === 'write'}
					...
					>
						...
						>
							<div
								class="flex {$mobile ? 'size-7' : 'size-5'} items-center justify-center self-center dark:hover:text-white transition m-0 touch-auto"
							>
